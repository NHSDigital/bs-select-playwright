...
@pytest.mark.rlpunit
def test_all_data_loaded(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, db_util) -> None:
...
def test_screening_unit_creation(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, db_util, unit_type, unit_status) -> None:   
    ...
    "unit_name": unit_name,
    "notes" : notes,
    **{day: str(random.randint(0, 999)) for day in ["sun", "mon", "tue", "wed", "thu", "fri", "sat"]}
    }
...
@pytest.mark.rlpunit
def test_screening_unit_cancellation(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, db_util) -> None:   
    ...
    page.wait_for_timeout(3000)
    rlp_screening_unit_list_page.verify_unit_has_no_matching_records_available_in_the_table(unit_name)
...
def test_defaults_are_set_correctly(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage) -> None:
    ...
    expect(page.locator("//input[@name='statusText' and @type='radio' and @value='ACTIVE']")).to_be_checked()
    expect(page.locator("//input[@name='statusText' and @type='radio' and @value='INACTIVE']")).not_to_be_checked()
...
def test_validate_add_new_unit_list_name_field(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, input_length) -> None:
    """Enters data into Screening Unit Name field on the 'Add a new unit' screen using input name lengths are 3 and 50  """
    # Logged into BSS_SO1
    UserTools().user_login(page, "BSO User1 - BS1")
    MainMenuPage(page).select_menu_option("Round Planning", "Screening Unit List")
    
    unit_name = test_helpers.generate_random_string(input_length) #this method will generate string between 3 & 100 which will be unique each time
    rlp_screening_unit_list_page.verify_unit_has_no_matching_records_available_in_the_table(unit_name)
...
@pytest.mark.rlpunit
def test_negative_non_integer_values_and_empty_unit_type(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage) -> None:
    ...
    "unit_name": "",
    "thu" : "",
    **{day: ''.join(random.choice(string.ascii_letters) for _ in range(2)) for day in ["sun", "mon", "tue", "wed", "fri", "sat"]}
    }
    rlp_screening_unit_list_page.add_screening_unit(unit_data, unit_type = "", unit_status = "")
    rlp_screening_unit_list_page.click_add_screening_unit_btn_on_pop_up_window()
    ...
    for day in ["Sun", "Mon", "Tues", "Wednes", "Thurs", "Fri", "Satur"]:
        expect(page.get_by_text(f"{day}day Appointments must be between 0 and 999")).to_be_visible()
...
@pytest.mark.rlpunit
def test_notes_shows_err_msg_for_invalid_chars(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage) -> None:
    ...
    rlp_screening_unit_list_page.click_add_screening_unit_btn_on_pop_up_window()
    expect(page.locator("//p[@id='error_notesText' and text()='Notes contains invalid characters']")).to_be_visible()
...
@pytest.mark.rlpunit
def test_add_screeing_unit_with_defaults(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage) -> None:
...
## Test_11 ##
@pytest.mark.rlpunit
def test_another_user_from_another_bso_creates_the_unit(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, context) -> None:
...
## Test_12 ##
@pytest.mark.rlpunit
def test_user2_from_same_bso_can_access_the_user1_data(page : Page,
    rlp_screening_unit_list_page: ScreeningUnitListPage, context) -> None:
