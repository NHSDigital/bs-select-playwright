...
def test_amend_screening_unit_name_verify_the_paging_count(
    page: Page, rlp_unit_list_page: ScreeningUnitListPage, db_util
) -> None:
    """
    ...
    ui_row_count = rlp_unit_list_page.extract_paging_unit_list_count()
    ...
    rlp_unit_list_page.filter_unit_by_name(unit_name)
    rlp_unit_list_page.dbl_click_on_filtered_unit_name()
    amend_unit_name = f"amend_name-{datetime.now()}"
    rlp_unit_list_page.enter_amend_screening_unit_name(amend_unit_name)
    rlp_unit_list_page.click_amend_screening_unit_btn_on_pop_up_window()
    page.wait_for_timeout(3000)
    ScreenshotTool(page).take_screenshot("rlp_unit_amend_tc_16_18_21.1")
    amend_db_row_count = rlp_unit_list_page.screening_unit_list_count_in_db(
        db_util
    )
    amend_ui_row_count = rlp_unit_list_page.extract_paging_unit_list_count()
...
def test_amend_screening_unit_name_using_min_max_char_length(
    page: Page, rlp_unit_list_page: ScreeningUnitListPage, input_length
) -> None:
    ...
    rlp_unit_list_page.enter_amend_screening_unit_name(amend_unit_name)
    rlp_unit_list_page.select_amend_unit_status_radio_btn("INACTIVE")
    rlp_unit_list_page.select_amend_unit_type_radio_btn("STATIC")
    rlp_unit_list_page.click_amend_screening_unit_btn_on_pop_up_window()
    page.wait_for_timeout(3000)
    ...
    rlp_unit_list_page.verify_screening_unit_by_name(amend_unit_name)


## Scenario 4 - 16.2.3 ##
def test_cancel_amend_screening_unit(
    page: Page, rlp_unit_list_page: ScreeningUnitListPage, db_util
) -> None:
    ...
    amend_unit_name = f"amend_name-{datetime.now()}"
    rlp_unit_list_page.enter_amend_screening_unit_name(amend_unit_name)
    rlp_unit_list_page.click_cancel_btn_on_amend_screening_unit_pop_up_window()
    page.wait_for_timeout(3000)
...
def test_amend_screening_unit_days_and_notes(
    ...
    rlp_unit_list_page.enter_amend_screening_unit_name(amend_unit_name)
    rlp_unit_list_page.enter_amend_usual_number_of_appointments_text_box(
        unit_data
    )
    rlp_unit_list_page.enter_amend_screening_unit_notes(amend_notes)
    rlp_unit_list_page.click_amend_screening_unit_btn_on_pop_up_window()
    page.wait_for_timeout(3000)
    rlp_unit_list_page.filter_unit_by_name(amend_unit_name)
    rlp_unit_list_page.dbl_click_on_filtered_unit_name()
    amended_day_values = rlp_unit_list_page.get_day_appointment_value()
...
def test_negative_amend_screening_unit_name(
    page: Page, rlp_unit_list_page: ScreeningUnitListPage, amend_unit_name
) -> None:
    """
    ...
    rlp_unit_list_page.amend_screening_unit(unit_data)
...
def test_user2_from_same_bso_can_access_the_user1_amended_data(
    page: Page, rlp_unit_list_page: ScreeningUnitListPage, context
) -> None:
    """
