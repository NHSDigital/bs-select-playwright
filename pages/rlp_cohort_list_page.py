...
class CohortListPage():
    ...
    def __init__(self, page : Page) -> None:
        ...
        self.create_screening_cohort_by_gp_practice_btn = page.locator("#addCohortByPracticeButtonText")
        self.screening_cohort_name_txtbox = page.locator("//input[@id='description']")
        self.default_screening_location_dropdown = page.locator("select#defaultLocation")
        ...
        self.done_btn_gp_practices_incluse_popup = page.locator("button#cancelButtonInAmendCohortPopup")
        self.screening_cohort_name_filter = page.locator("th#descriptionFilter > input")
        self.screening_location_filter = page.locator("th#defaultLocationFilter > input")
        ...
        self.amend_select_gp_practices_btn = page.locator("span#selectElementsButtonText")
        ...
        self.amend_done_btn_gp_practices = page.locator("button#cancelButtonInAmendCohortPopup")
        ...
        self.create_screening_cohort_by_outcode_btn = page.locator("button#addCohortByOutcodeButton")
    ...
    def select_amend_screening_location_dropdown(self, location_name:str) -> CohortListPage:
    ...
    def enter_amend_expected_attendance_rate(self, attendance_rate:int) -> CohortListPage:
    ...
    def select_default_screening_location_dropdown(self, location_name:str) -> CohortListPage:
    ...
    def click_add_btn_to_select_outcode(self, outcode) -> CohortListPage:
        outcode_add_btn = self.page.locator(f"//tr[td[1][normalize-space()='{outcode}']]//button[text()='Add']")
    ...
    # Use an XPath selector to find the "Remove" button in the same row where the first column text matches the gp_code
        remove_button = self.page.locator(f"//table[@id='practicesToIncludeList']//tr[td[normalize-space()='{gp_code}']]//button[text()='Remove']")
        remove_button.click()
        return self
    ...
    def create_cohort_user2_bso2(self, cohort_name, location_name) -> CohortListPage:
        ...
        self.enter_screening_cohort_name_field(cohort_name).enter_expected_attendance_rate("25")
        self.select_default_screening_location_dropdown(location_name).select_default_screening_unit_dropdown("Batman")
        self.click_select_gp_practices_btn().enter_gp_code_field("A00009").click_add_btn_gp_practices_to_include().click_done_btn_gp_practices_include_popup()
    ...
    def create_cohort(self, cohort_name, location_name) -> CohortListPage:
        ...
        self.enter_screening_cohort_name_field(cohort_name).enter_expected_attendance_rate("25")
        self.select_default_screening_location_dropdown(location_name).select_default_screening_unit_dropdown("Batman")
        self.click_select_gp_practices_btn().enter_gp_code_field("A00005").click_add_btn_gp_practices_to_include().click_done_btn_gp_practices_include_popup()
        self.click_create_screening_cohort_save_btn()
        self.page.wait_for_timeout(3000)
        return self
    
    def create_cohort_without_gp(self, cohort_name, location_name, unit_name) -> CohortListPage:
        self.click_create_screening_cohort_by_gp_practice_btn()
        self.enter_screening_cohort_name_field(cohort_name).enter_expected_attendance_rate("25")
        self.select_default_screening_location_dropdown(location_name).select_default_screening_unit_dropdown(unit_name)
        self.click_create_screening_cohort_save_btn()
        self.page.wait_for_timeout(3000)
        return self    
    
    def create_cohort_outcode_without_gp(self, cohort_name, attendance_rate, location_name, unit_name) -> CohortListPage:
        self.click_create_screening_cohort_by_outcode_btn()
        self.enter_screening_cohort_name_field(cohort_name).enter_expected_attendance_rate(attendance_rate)
        self.select_default_screening_location_dropdown(location_name).select_default_screening_unit_dropdown(unit_name)
    ...
    def extract_cohort_paging_info(self) -> int:
        ...
        self.page.wait_for_timeout(3000)
        re_search_result = re.search('Showing (\d+) to (\d+) of (\d+) entries', paging_info_text)
        return (int(re_search_result.group(3)))
    
    def screening_cohorts_count_in_db(self, db_util)->int:
        result = db_util.get_results("""select count(1) 
    ...
    def create_unit_if_not_exists(self, unit_name: str) -> None:
        ...
        self.page.wait_for_selector(f"//input[@id='unitTypeText' and @type='radio' and @value='MOBILE']").check()
        ...
        try:
            ...
            if "Name is already in use by another unit" in self.error_message_locator.inner_text():
